...
<head>
	<meta charset="utf-8">
	<title>P&aacute;gina consultar afiliaci&oacute;n</title>
	...
	<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
		...
		$(function() {
				...
				//CAMBIA EL VALOR SELECCIONADO EN EL CONTROL SELECT
				$(document).ready(function(){
		    		var opcion = document.getElementById("nciudad").value;
				    $("#sCiudad").val(opcion);
				    $('#sCiudad').change();
				});
				//AL CAMBIAR EL DOCUMENTO ACTUALIZA VALOR DEL CAMPO HIDDEN
		    	$(document).on('change', '#sCiudad', function(event) {
		    		$('#nciudad').val($("#sCiudad:selected").text());
		    	});
		});
	</script>      	
</head>
	...
	<div id="page" class="single_page">
		...
		<section class="breadcrumbs_block clearfix parallax">
			<div class="container center">
				<h2><b>Datos</b> de la afiliaci&oacute;n</h2>
				<p>Informaci&oacute;n  de la afiliaci&oacute;n</p>
			</div>
		</section><!-- //BREADCRUMBS -->
		<!-- BLOG -->
		<section id="blog">
				...
				<div class="row">
					...
					<div class="blog_block col-lg-9 col-md-9 padbot50">
						...
						<div class="blog_post margbot50 clearfix" data-animated="fadeInUp">
								<!-- FORMULARIO CONSULTAR AFILIACION-->
								<div id="fields">
								      <form:form class="clearfix" method="GET" action="">
								      		<legend>FORMULARIO CONSULTAR AFILIACION</legend>
          									<table>
              									<tr>
	             									<td><form:label path="lCodigo">C&oacute;digo: </form:label></td>
                  									<td><form:input size="110" path="lCodigo" value="${codigo_afiliacion}"/></td>
              									</tr>
              									<tr>
	             									<td><form:label path="lClientePadre">Cliente padre: </form:label></td>
                  									<td><form:input size="110" path="lClientePadre" value="${cpadre_afiliacion}"/></td>
              									</tr>
              									<tr>
	             									<td><form:label path="lClienteHijo">Cliente hijo: </form:label></td>
                  									<td><form:input size="110" path="lClienteHijo" value="${chijo_afiliacion}"/></td>
              									</tr>
              									<tr>
                  									<td><form:label path="sEstado">Estado: </form:label></td>
                  									<td>
                  										<form:select id="sEstado" path="sEstado" style="min-width:780px;height:37px;">
              											<form:option value="NONE" label="Seleccione una opción"/>
              											<form:options items="${lstEstados}" />
														</form:select>
														<input type="hidden" id="nestado" name="hestado" value="${estado_afiliacion}"/>
              										</td>
              									</tr>
              									<tr>
                  									<td><form:label path="sCiudad">Ciudad: </form:label></td>
                  									<td>
                  										<form:select id="sCiudad" path="sCiudad" style="min-width:780px;height:37px;">
              											<form:option value="NONE" label="Seleccione una opción"/>
              											<form:options items="${lstCiudades}" />
														</form:select>
														<input type="hidden" id="nciudad" name="hciudad" value="${ciudad_afiliacion}"/>
              										</td>
              									</tr>
              									<tr>
                  									<td><form:label path="sFecha">Fecha: </form:label></td>
                  									<td><form:input size="110" path="sFecha" value="${fecha_afiliacion}"/></td>
              									</tr>
              									<tr>
                  									<td><form:label path="sHora">Hora: </form:label></td>
                  									<td><form:input size="110" path="sHora" value="${hora_afiliacion}"/></td>
              									</tr>
              									<tr align="center">
                  									<td colspan="2">
...
<%
	session.removeAttribute("codigo_afiliacion");
	session.removeAttribute("cpadre_afiliacion");
	session.removeAttribute("chijo_afiliacion");
	session.removeAttribute("estado_afiliacion");
	session.removeAttribute("ciudad_afiliacion");
	session.removeAttribute("fecha_afiliacion");
	session.removeAttribute("hora_afiliacion");
