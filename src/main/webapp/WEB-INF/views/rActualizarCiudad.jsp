...
<head>
	<meta charset="utf-8">
	<title>P&aacute;gina ciudad</title>
	...
	<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
		...
		$(function() {
				...
				$(document).ready(function(){
		    		var opcion = document.getElementById("ndepto").value;
				    $("#sDepartamento").val(opcion);
				    $('#sDepartamento').change();
				});
				//AL CAMBIAR EL DOCUMENTO ACTUALIZA VALOR DEL CAMPO HIDDEN
		    	$(document).on('change', '#sDepartamento', function(event) {
		    		$('#nDepto').val($("#sDepartamento:selected").text());
		    	});
		});
	</script>      	
</head>
	...
	<div id="page" class="single_page">
		...
		<section class="breadcrumbs_block clearfix parallax">
			<div class="container center">
				<h2><b>Datos</b> de la ciudad</h2>
				<p>Diligencie los datos de la ciudad</p>
			</div>
		</section><!-- //BREADCRUMBS -->
		<!-- BLOG -->
		<section id="blog">
				...
				<div class="row">
					...
					<div class="blog_block col-lg-9 col-md-9 padbot50">
						...
						<div class="blog_post margbot50 clearfix" data-animated="fadeInUp">
								<!-- FORMULARIO CIUDAD -->
								<div id="fields">
								      <c:url value="/r2ActualizarCiudad" var="formUrl"/>
								      <form:form id="actciudad" class="clearfix" method="GET" action="${formUrl}">
								      		<legend>FORMULARIO ACTUALIZAR CIUDAD</legend>
          									<table>
              									<tr>
	             									<td><form:label path="iCodigo">C&oacute;digo: </form:label></td>
                  									<td><form:input size="110" path="iCodigo" value="${codigo_ciudad}"/></td>
              									</tr>
              									<tr>
	             									<td><form:label path="sNombre">Nombre: </form:label></td>
                  									<td><form:input size="110" id="sNombre" path="sNombre" value="${nombre_ciudad}"/></td>
              									</tr>
              									<tr>
	             									<td><form:label path="sDepartamento">Departamento:</form:label></td> 
                  									<td>
                  										<form:select id="sDepartamento" path="sDepartamento" style="min-width:765px;height:37px;">
              											<form:option value="NONE" label="Seleccione una opción"/>
              											<form:options items="${lstDeptos}" />
														</form:select>
														<input type="hidden" id="ndepto" name="hdepto" value="${nombre_depto}"/>
              										</td>
              									</tr>
              									<tr>
                  									<td><form:label path="sFecha">Fecha: </form:label></td>
                  									<td><form:input size="110" path="sFecha" value="${fecha_ciudad}"/></td>
              									</tr>
              									<tr>
                  									<td><form:label path="sHora">Hora: </form:label></td>
                  									<td><form:input size="110" path="sHora" value="${hora_ciudad}"/></td>
              									</tr>
              									<tr align="center">
                  									<td colspan="2">
                  										<input type="submit" id="btnactualizar" value="Actualizar" class="btn btn-primary btn-lg"/>
