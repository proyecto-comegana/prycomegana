...
<head>
	<meta charset="utf-8">
	<title>P&aacute;gina consultar nodo</title>
	...
	<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
		...
		$(function() {
				//CAMBIA EL VALOR SELECCIONADO EN EL CONTROL SELECT
				$(document).ready(function(){
		    		var opcion = document.getElementById("nPosicion").value;
				    $("#iPosicion").val(opcion);
				    $('#iPosicion').change();
				});
				//AL CAMBIAR EL DOCUMENTO ACTUALIZA VALOR DEL CAMPO HIDDEN
		    	$(document).on('change', '#iPosicion', function(event) {
		    		$('#nPosicion').val($("#iPosicion:selected").text());
		    	});
				//CAMBIA EL VALOR SELECCIONADO EN EL CONTROL SELECT
				$(document).ready(function(){
		    		var opcion = document.getElementById("nEstado").value;
				    $("#sEstado").val(opcion);
				    $('#sEstado').change();
				});
				//AL CAMBIAR EL DOCUMENTO ACTUALIZA VALOR DEL CAMPO HIDDEN
		    	$(document).on('change', '#sEstado', function(event) {
		    		$('#nEstado').val($("#sEstado:selected").text());
		    	});
				//CAMBIA EL VALOR SELECCIONADO EN EL CONTROL SELECT
				$(document).ready(function(){
		    		var opcion = document.getElementById("nNivel").value;
				    $("#iNivel").val(opcion);
				    $('#iNivel').change();
				});
				//AL CAMBIAR EL DOCUMENTO ACTUALIZA VALOR DEL CAMPO HIDDEN
		    	$(document).on('change', '#iNivel', function(event) {
		    		$('#nNivel').val($("#iNivel:selected").text());
		    	});
		});
	</script>      	
</head>
	...
	<div id="page" class="single_page">
		...
		<section class="breadcrumbs_block clearfix parallax">
			<div class="container center">
				<h2><b>Datos</b> del nodo</h2>
				<p>Informaci&oacute;n  del nodo</p>
			</div>
		</section><!-- //BREADCRUMBS -->
		<!-- BLOG -->
		<section id="blog">
				...
				<div class="row">
					...
					<div class="blog_block col-lg-9 col-md-9 padbot50">
						...
						<div class="blog_post margbot50 clearfix" data-animated="fadeInUp">
								<!-- FORMULARIO CONSULTAR NODO -->
								<div id="fields">
								      <form:form class="clearfix" method="GET" action="">
								      		<legend>FORMULARIO CONSULTAR NODO</legend>
          									<table>
              									<tr>
	             									<td><form:label path="lCodigo">C&oacute;digo: </form:label></td>
                  									<td><form:input size="110" path="lCodigo" value="${codigo_nodo}"/></td>
              									</tr>
              									<tr>
	             									<td><form:label path="lCliente">Cliente: </form:label></td>
                  									<td><form:input size="110" path="lCliente" value="${cliente_nodo}"/></td>
              									</tr>
              									<tr>
	             									<td><form:label path="lArbol">Arbol: </form:label></td>
                  									<td><form:input size="110" path="lArbol" value="${arbol_nodo}"/></td>
              									</tr>
              									<tr>
	             									<td><form:label path="iPosicion">Posici&oacute;n:</form:label></td> 
                  									<td>
                  										<form:select id="iPosicion" path="iPosicion" style="min-width:780px;height:37px;">
              											<form:option value="0" label="Seleccione una opción"/>
              											<form:options items="${lstPosiciones}"/>
														</form:select>
														<input type="hidden" id="nPosicion" name="hPosicion" value="${posicion_nodo}"/>														
              										</td>
              									</tr>
              									<tr>
	             									<td><form:label path="sEstado">Estado:</form:label></td> 
                  									<td>
                  										<form:select id="sEstado" path="sEstado" style="min-width:780px;height:37px;">
              											<form:option value="0" label="Seleccione una opción"/>
              											<form:options items="${lstEstados}"/>
														</form:select>
														<input type="hidden" id="nEstado" name="hEstado" value="${estado_nodo}"/>														
              										</td>
              									</tr>
              									<tr>
	             									<td><form:label path="iNivel">Nivel:</form:label></td> 
                  									<td>
                  										<form:select id="iNivel" path="iNivel" style="min-width:780px;height:37px;">
              											<form:option value="0" label="Seleccione una opción"/>
              											<form:options items="${lstNiveles}"/>
														</form:select>
														<input type="hidden" id="nNivel" name="hNivel" value="${nivel_nodo}"/>														
              										</td>
              									</tr>
              									<tr>
                  									<td><form:label path="sFecha">Fecha: </form:label></td>
                  									<td><form:input size="110" path="sFecha" value="${fecha_nodo}"/></td>
              									</tr>
              									<tr>
                  									<td><form:label path="sHora">Hora: </form:label></td>
                  									<td><form:input size="110" path="sHora" value="${hora_nodo}"/></td>
              									</tr>
              									<tr align="center">
                  									<td colspan="2">
                  										<input type="submit" value="Regresar" class="btn btn-primary btn-lg"/>
                 									</td>
              									</tr>
          									</table>
      									</form:form>
...
<%
	session.removeAttribute("codigo_pais");
	session.removeAttribute("nombre_pais");
	session.removeAttribute("fecha_pais");
	session.removeAttribute("hora_pais");
%>
</body>
</html>